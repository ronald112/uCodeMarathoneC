...
t_warshall **mx_set_start_vars(t_main *vars, t_graph *graph) {
    ...
    for (int i = 0; i < vars->nmb_isld; i++) {
        for (int j = 0; j < vars->nmb_isld; j++) {
            warshall[i][j].path = mx_strdup(graph->isld[i]);
        }
    }
    return warshall;
}

static void mx_add_path_to_node(t_warshall **warshall, t_main *vars, t_graph *graph) {
    int i = vars->rws;
    int j = vars->clmn;
    int k = vars->cnt_loop;

    warshall[i][j].path = govno(warshall[i][j].path, warshall[i][k].path);
    warshall[i][j].path = govno(warshall[i][j].path, vars->add_arow);
    warshall[i][j].path = govno(warshall[i][j].path, warshall[k][j].path);

    warshall[i][j].cos_path = govno(warshall[i][j].cos_path, graph->weight[i][k]);
    warshall[i][j].cos_path = govno(warshall[i][j].cos_path, vars->add_plus);
    warshall[i][j].cos_path = govno(warshall[i][j].cos_path, graph->weight[k][j]);
}

static t_warshall *mx_add_paral_path(t_warshall **warsh_nxt, t_main *vars, t_graph *graph) {
    int i = vars->rws;
    int j = vars->clmn;
    int k = vars->cnt_loop;
    warsh_nxt[i][j].next = (t_warshall*)malloc(sizeof(t_warshall));
    warsh_nxt[i][j].next->next = NULL;

    graph->weight[i][j] = graph->weight[i][k] + graph->weight[k][j];

    mx_add_path_to_node(warsh_nxt[i][j].next, vars, graph);

    if (warsh_nxt[i][k].next->next != NULL)
        mx_add_paral_path(warsh_nxt[i][j].next->next, vars, graph);

    return warsh_nxt[i][j].next;
}

static void mx_add_path(t_warshall **warshall, t_main *vars, t_graph *graph) {
    int i = vars->rws;
    int j = vars->clmn;
    int k = vars->cnt_loop;
    graph->weight[i][j] = graph->weight[i][k] + graph->weight[k][j];

    mx_add_path_to_node(warshall, vars, graph);

    if (warshall[i][k].next != NULL) {

        warshall[i][j].next = mx_add_paral_path(warshall, vars, graph);
    }
}

t_warshall *mx_set_last_node(t_warshall *tmp) {
	while(tmp -> next != NULL)
		tmp = tmp -> next;
	return tmp;
}
...
static void mx_set_path(t_warshall **warshall, t_main *vars, t_graph *graph) {
    ...
    if (graph->weight[i][j] > graph->weight[i][k] + graph->weight[k][j]) {
        ...
        mx_add_path(warshall, vars, graph);
    }
    else if (graph->weight[i][j] == graph->weight[i][k] + graph->weight[k][j]) {
        warshall[i][j].next = (t_warshall*)malloc(sizeof(t_warshall));
        warshall[i][j].next->next = NULL;
        mx_add_path(warshall[i][j].next, vars, graph);
    }
    ...
    for (vars->cnt_loop = 0; vars->cnt_loop < vars->nmb_isld; vars->cnt_loop++) {
        for (vars->rws = 0; vars->rws < vars->nmb_isld; vars->rws++) {
            for (vars->clmn = 0; vars->clmn < vars->nmb_isld; vars->clmn++) {
                ...
                && graph->weight[vars->rws][vars->clmn] != INT_MAX) {
